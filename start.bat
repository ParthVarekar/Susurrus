...
if not "!MODEL_CHOICE!"=="5" if not "!MODEL_CHOICE!"=="4" (
    ...
    if not exist "!LOCAL_MODEL_PATH!" (
        echo [DOWNLOAD] Auto-downloading !TARGET_MODEL_NAME! from HuggingFace repository !HF_REPO!...
        python -c "from huggingface_hub import hf_hub_download; print('Downloading...'); hf_hub_download(repo_id='!HF_REPO!', filename='!HF_FILE!', local_dir=r'%~dp0models')"
        if !ERRORLEVEL! NEQ 0 (
            color 0C
            echo [ERROR] Model download failed. Please check your internet connection.
            pause
            exit /b 1
        )
        echo [OK] Model downloaded successfully to models/!TARGET_MODEL_NAME!
    ) else (
        echo [OK] Model file found: models/!TARGET_MODEL_NAME!
    )
)
...
if not "!MODEL_CHOICE!"=="5" (
    ...
    if !ERRORLEVEL! EQU 0 (
        ...
        echo [OK] llama-server started in background window. Waiting for model load (10-15s)...
